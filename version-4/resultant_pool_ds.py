...
class SelectOrm:
    ...
    def execute(self):
        ...
        temp_select_data = ResultantSet()
        if self.__field:
            
            for index, field in enumerate(self.__field):
                temp_select_data.__meta__[f'{self.__model.__classname__}.{field.name}'] = index
            ...
            for obj in self.__filtered:
                ...
                for field in self.__field:
                    if field.name in temp:
                        field_data.append(temp[field.name].get_value())
                
                temp_select_data.data.append(tuple(field_data))
            ...
            for obj in self.__filtered:
                ...
                if start:
                    for index, field in enumerate(temp.values()):
                        temp_select_data.__meta__[field.name] = index
                    start = False
                
                for value in temp.values():
                    field_data.append(value.get_value())
    ...
    def __init__(self, **kwargs):
        for field_name, field_value in self.__class__.__dict__.items():
            if isinstance(field_value, Field):
                ...
                field_copy.set_name(field_name)
...
# select Query
query = SelectOrm()
res = query.select(User.id, User.name).table(User).where([_eq(User.name, "B")]).order_by([_asc(User.name)]).execute()
print(res.data)
print(res.__meta__)

res = query.select(Orders.id, Orders.name, Orders.userid).table(Orders).where([]).order_by([_asc(User.name)]).execute()
print(res.data)
print(res.__meta__)
