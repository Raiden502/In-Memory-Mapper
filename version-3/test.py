...
res = insertQuery.insert(User).fields([
    {"name": "John", "data": 1, "num":2},
    {"name": "John", "data": 1, "num":2},
    {"name": "John", "data": 2, "num":2},
    {"name": "Jane", "data": 1, "num":2},
])

res.execute()

query = Orm()
res = query.select(User, User.name, User.data).where([Operators._eq(User.name, "Jane"),Operators._eq(User.data, 2), Operators._eq(User.num, 2)]).order_by([Operators._asc(User.name)])
print(res.execute())